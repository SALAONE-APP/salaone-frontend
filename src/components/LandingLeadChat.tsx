  ...
  return <aside ref={panelRef} role="dialog" aria-modal="false" aria-labelledby="lead-chat-title" className="fixed bottom-20 left-3 right-3 z-50 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-2xl border border-neutral-700 bg-neutral-950 text-white shadow-2xl shadow-black/60 sm:left-auto sm:right-5 sm:w-[400px]">
    ...
    <div className="bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,0.08),transparent_40%)] p-4">
      ...
      <form onSubmit={submit} className="space-y-3" noValidate>
        {fields.map(([field, label, placeholder, type]) => <div key={field} className="rounded-xl border border-neutral-800 bg-black/40 px-4 py-3.5"><label htmlFor={`lead-${field}`} className="mb-2 block text-sm font-medium text-neutral-300">{label}{field !== "cnpj" && <span className="text-brand-pink"> *</span>}</label><input ref={field === "name" ? nameRef : undefined} id={`lead-${field}`} type={type} value={form[field]} placeholder={placeholder} disabled={submitting} autoComplete={field === "name" ? "name" : field === "phone" ? "tel" : field === "email" ? "email" : "off"} inputMode={field === "phone" || field === "cnpj" ? "numeric" : undefined} aria-invalid={Boolean(errors[field])} aria-describedby={errors[field] ? `lead-${field}-error` : undefined} onChange={(event) => change(field, field === "phone" ? maskPhone(event.target.value) : field === "cnpj" ? maskCnpj(event.target.value) : event.target.value)} className="landing-lead-input h-11 w-full border-0 px-3 text-base outline-none placeholder:text-neutral-500 disabled:opacity-60" />{errors[field] && <p id={`lead-${field}-error`} role="alert" className="mt-2 text-xs text-red-400">{errors[field]}</p>}</div>)}
